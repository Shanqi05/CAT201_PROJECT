...
const DonationPage = () => {
    // [FIX] Renamed 'address' -> 'houseNo' and 'jalan' -> 'street' to match Backend Servlet
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-8">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    {/* [FIX] Updated inputs to use houseNo and street */}
                                    <div>
                                        <label className="block text-xs font-bold text-gray-500 uppercase mb-1">House No / Taman</label>
                                        <input name="houseNo" value={formData.houseNo} onChange={handleChange} className={inputClass('houseNo')} />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Street (Jalan)</label>
                                        <input name="street" value={formData.street} onChange={handleChange} className={inputClass('street')} />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="border-2 border-gray-300 rounded-xl p-1 bg-white">
                                        ...
                                        >
                                            <div className="flex items-center gap-2">
                                                <span className="font-bold text-gray-500 uppercase text-xs">Select Genres</span>
                                        ...
                                        {showGenres && (
                                            <div className="p-3 border-t-2 border-gray-100 mt-1 max-h-60 overflow-y-auto bg-gray-50 rounded-b-lg">
                                                <div className="flex flex-wrap gap-2">
                                                    {genreOptions.map(genre => (
                                                        <label key={genre} className={`flex items-center space-x-1 cursor-pointer px-3 py-1 rounded-full border transition-all ${formData.genres.includes(genre) ? 'bg-purple-100 border-purple-300 text-purple-700' : 'bg-white border-gray-200 hover:border-gray-300'}`}>
                                                            <input type="checkbox" value={genre} checked={formData.genres.includes(genre)} onChange={handleGenreChange} className="hidden" />
                                    ...
                                    <div>
                                        ...
                                        <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-gray-50 hover:bg-gray-100 transition-all hover:border-purple-400 relative overflow-hidden group">
                                            {previewUrl ? <img src={previewUrl} className="w-full h-full object-cover" /> : <div className="text-center"><Upload className="mx-auto text-gray-400 mb-1"/><span className="text-sm text-gray-500">Upload Photo</span></div>}
                            ...
                            <button type="submit" disabled={isSubmitting} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white font-black py-4 rounded-xl hover:shadow-2xl transition-all">
